...
| MetaMask: &ldquo;Could not fetch chain ID. Is your RPC URL correct?&rdquo; (Docker, host anvil works) | Anvil rejecting MetaMask&rsquo;s `chrome-extension://` origin. The compose file passes `--allow-origin "*"` for this. If you&rsquo;re on a fork that doesn&rsquo;t, add it. Verify with `curl -X POST http://127.0.0.1:8545 -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","method":"eth_chainId","params":[],"id":1}'` &mdash; should return `"result":"0x7a69"`. |
| MetaMask: &ldquo;Could not fetch chain ID. Is your RPC URL correct?&rdquo; (manual entry) | RPC URL is missing the `http://` prefix. Use `http://127.0.0.1:8545`, not `127.0.0.1:8545` and not `localhost:8545`. Or click the **Add / switch to Anvil** button on the page. |
